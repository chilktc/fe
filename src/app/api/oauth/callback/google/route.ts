  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 400 && errorData.code === "CLIENT_ERROR") {
        ...
        if (errorData.code === "OAUTH_TOKEN_EXCHANGE_FAILED")
          errorParam = "token_exchange_failed";
        if (errorData.code === "OAUTH_USERINFO_FAILED")
          errorParam = "user_info_failed";
      } else if (
        response.status === 403 &&
        errorData.code === "OAUTH_EMAIL_NOT_REGISTERED"
      ) {
    ...
    // firstLogin 여부에 따라 목적지 결정
    const redirectUrl = firstLogin
      ? new URL("/login/terms", request.url)
      : new URL("/", request.url);
